...
public class Service {
	...
	public static void purchaseProduct(ArrayList<Product> productList) throws Exception {
		...
		for (Product product : productList) {
			if (enteredProductCode.equalsIgnoreCase(product.getProductCode())) {
				...
				if (dateEntered.compareTo(dateValid) <= 0)
					activeState = "active";
				else if (dateEntered.compareTo(dateOpen) >= 0)
					activeState = "active";
				else
		...
		if (dateEntered.after(dateOpen) && dateEntered.before(dateValid) && dateEntered.before(dateExpiry) ||dateEntered.compareTo(dateOpen)==0 || dateEntered.compareTo(dateValid)==0  )
